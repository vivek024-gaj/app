...
import in.cropdata.app.error.ApiError;
...
public class GlobleRestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleInternalAuthenticationServiceException(final InternalAuthenticationServiceException ex, final WebRequest request) {
		logger.info(ex.getClass().getName());
		logger.error("error", ex);
		//
		final ApiError apiError = new ApiError(HttpStatus.BAD_REQUEST, "Username and Password Incorrect",
				"error occurred");
		return new ResponseEntity<Object>(apiError, new HttpHeaders(), apiError.getStatus());
	}
